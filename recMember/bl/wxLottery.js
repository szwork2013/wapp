...
obj.getLotteryByEname = function(lotteryEname,cb){
	if(!lotteryEname) return cb('no uid');
	lotteryModel.insertOneByObj({
		ename:lotteryEname
	},function(err,doc){
		cb(err,doc)
	})
...
obj.getUserLotteryRec = function(uid,skip,pagesize,cb){ //根据用户Id查找他的抽奖记录
	var cb = cb || function(){}
	var skip = skip || 0;
...
obj.getUserLotteryRecById = function(uid, lotteryId, skip, pagesize, cb){ //根据用户Id和抽奖Id查找他的抽奖记录
	var cb = cb || function(){};
	var skip = skip || 0;
	var pagesize = pagesize || 50; 
...
obj.startLottery = function(userId, lotteryId, recordIp, cb){ //用户进入抽奖页面点击抽奖程序
	var cb = cb || function(){}
...
obj._getPrize = function(userId, lotteryId, recordIp, cb){//用户进入抽奖程序
	...
	lotteryPrizeModel.findByObj({//获得奖品列表
			...
			if(!prList || prList.length == 0){ //没有找到奖品，表示此次抽奖未中奖
				return obj._completeLottery(userId, lotteryId, recordIp, 0, 0, cb);//表示没有抽到奖品
			}
			...
			if(prizeLen>0){
				...
				return obj._completeLottery(userId, lotteryId, recordIp, userPrizeId, 0, cb);//抽到奖品 userPrizeId
			}
			...
			lotteryRecModel.countAll({ //查找此 抽奖活动 的抽奖记录数
				...
				return obj._completeLottery(userId, lotteryId, recordIp, userCountPrizeId, nextPriceNum, cb);//表示没有抽到奖品
...
obj._completeLottery = function(userId, lotteryId, recordIp, prizeId, nextPriceNum, cb){//用户抽奖结束，更新记录和相关信息
