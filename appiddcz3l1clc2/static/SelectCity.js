/*定义省份数组*/
provinceArray = new Array("湖北省", "上海市", "天津市", "重庆市", "河北省", "山西省", "内蒙古自治区", "辽宁省", "吉林省", "黑龙江省", "江苏省", "浙江省", "安徽省", "福建省", "江西省", "山东省", "河南省", "北京市", "湖南省", "广东省", "广西壮族自治区", "海南省", "四川省", "贵州省", "云南省", "西藏自治区", "陕西省", "甘肃省", "宁夏回族自治区", "青海省", "新疆维吾尔族自治区", "香港特别行政区", "澳门特别行政区", "台湾省", "其它");

/*定义城市数组*/
cityArray = new Array();
cityArray[0] = new Array("湖北省", "武汉|宜昌|荆州|襄樊|黄石|荆门|黄冈|十堰|恩施|潜江|天门|仙桃|随州|咸宁|孝感|鄂州");
...
cityArray[17] = new Array("北京市", "东城|西城|崇文|宣武|朝阳|丰台|石景山|海淀|门头沟|房山|通州|顺义|昌平|大兴|平谷|怀柔|密云|延庆");
...
function initProvince() {
    ...
    //alert($("#selProvince").find('option').length);
    //$("#selProvince").find('option').length=0
    $("#selProvince").html("");
    ...
    for (i = 0; i < provinceArray.length; i++) {
        ...
        $("#selProvince").append("<option value='" + provinceArray[i] + "'>" + provinceArray[i] + "</option>");
    }
}

/*获取城市列表*/
function getCity(currentProvince) {
    //当前 所选择 的 省
    ...
    for (i = 0; i < cityArray.length; i++) {
        //得到 当前省 在 城市数组中的位置
        if (cityArray[i][0] == currentProvince) {
            //得到 当前省 所辖制的 地市
            tmpcityArray = cityArray[i][1].split("|");
            k = tmpcityArray[0];
            for (j = 0; j < tmpcityArray.length; j++) {
                //填充 城市 下拉选单
                $("#selCity").append("<option value='" + tmpcityArray[j] + "'>" + tmpcityArray[j] + "</option>"); 
                
                //document.all.selCity.options[document.all.selCity.length] = new Option(tmpcityArray[j], tmpcityArray[j]);
                //$("#selCity option[index='0']").prepend("<option value='" + tmpcityArray[j] + "'>" + tmpcityArray[j] + "</option>")
            }
        }
    }
    /*设置显示默认值*/
    $("#selCity").siblings(".ui-btn-inner").children(".ui-btn-text").html("请选择");
//    if (currentProvince == "") {
//        $("#selCity").siblings(".ui-btn-inner").children(".ui-btn-text").html("请选择");
//    }
//    else {
//        $("#selCity").siblings(".ui-btn-inner").children(".ui-btn-text").html(k);
//    }
    checkArea();
}
