var userDl = require('../dl/userDl.js');
...
obj.read = function(req, res){
	...
	userDl.findAll(filter, skip, pageSize, function(err,doc){
		if(err) return res.send(500,err);
		if(!doc) return res.json(resObj);
		resObj["Data"] = doc;

		userDl.countAll(filter,function(err,count){
			if(err) return res.send(500,err);
			resObj["Total"] = count
			res.json(resObj);
		})
...
obj.update = obj.create = function(req, res){
	...
	userDl.createOneOrUpdate(query, req.models[0], function(err, doc){
		if(err) return res.send(500,err);
...
obj.destroy = function(req, res){
	...
	userDl.destroy(query, function(err, doc){
		if(err) return res.send(500,err);
		if(!doc) return res.json([])
		res.json(doc);
	})
