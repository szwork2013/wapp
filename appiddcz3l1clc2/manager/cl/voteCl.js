...
obj.list = function(req, res){
	res.render('vote_list', {session:req.session});
}
obj.read = function(req, res){
	...
	voteDl.findAll(filter, skip, pageSize, function(err,doc){
		if(err) return res.send(500,err);
		if(!doc) return res.json(resObj);
		resObj["Data"] = doc;

		voteDl.countAll(filter,function(err,count){
			if(err) return res.send(500,err);
			resObj["Total"] = count
			res.json(resObj);
		})
...
obj.update = obj.create  = function(req, res){
	...
	voteDl.createOneOrUpdate(query, req.models[0], function(err, doc){
		if(err) return res.send(500,err);
...
obj.destroy = function(req, res){
	...
	voteDl.destroy(query, function(err, doc){
		if(err) return res.send(500,err);
		if(!doc) return res.json([])
		res.json(doc);
	})
...
obj.getGroup = function(req, res){
	...
	filter["voteId"] = "0";
	voteDl.findAll(filter, 0, 1000, function(err,doc){
		if(err) return res.send(500,err);
		if(!doc) return res.json([])
		res.json(doc);
	})
