var clientDl = require('../dl/clientDl.js');
var utils = require('../../lib/utils.js')
var count = 0;
var obj = {};
var salt = global.app.get('salt');

obj.list = function(req, res){
	res.render('client_list', {session:req.session});
}
obj.read = function(req, res){
	var filter =  utils.kendoToMongoose(req.body.filter, req.session.clientId);
	var skip = req.body.skip || 0;
	var pageSize = req.body.pageSize || 20;
	var resObj = {"Data":[],"Total":0};

	if(filter["clientId"]){
		var clientId = filter["clientId"];
		filter["_id"] = clientId;
		delete filter["clientId"];
	}

	clientDl.findAll(filter, skip, pageSize, function(err,doc){
		if(err) return res.send(500,err);
		if(!doc) return res.json(resObj);
		resObj["Data"] = doc;

		clientDl.countAll(filter,function(err,count){
			if(err) return res.send(500,err);
			resObj["Total"] = count
			res.json(resObj);
		})
		
	})
}

obj.update = obj.create = function(req, res){

	var query;
	if(!req.models[0].clientKey){
		req.models[0].clientKey = utils.md5((Date.now()+(count++)).toString())
	}
	if(req.models[0]["_id"]){
		query = {"_id": req.models[0]["_id"]};
	}
	else{
		query = {"clientKey": ''};
	}
	delete req.models[0]["_id"];

	if(req.models[0].password && req.models[0].password.length !== 32){
		req.models[0].password = utils.md5(req.models[0].password + salt); 
	}
	if(req.models[0].userName && req.models[0].userName === global.app.get('widgetadmin')){
		return res.send(500);
	}

	clientDl.createOneOrUpdate(query, req.models[0], function(err, doc){
		if(err) return res.send(500,err);
		if(!doc) return res.json([])
		res.json(doc);
	})
}
obj.destroy = function(req, res){
	var query = {"_id": req.models[0]["_id"]};
	clientDl.destroy(query, function(err, doc){
		if(err) return res.send(500,err);
		if(!doc) return res.json([])
		res.json(doc);
	})
}


module.exports = obj;