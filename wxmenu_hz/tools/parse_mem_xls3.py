...
def excel_table_byindex(file= 'file.xls',colnameindex=0,by_index=2):
    ...
    list =[]
    key = [
        'hongli', 'xingbie', 'xingbie2', 'jiaofeiqijian', 'baozhangqijian',
        'toubaonianling', 'toubaoniandu', 'baofei', 'zhonglhongli',
        'youxiaobaoxjine', 'gudingbaofeitiaozheng'
    ]
         ...
         if row:
             ...
             for i in range(len(colnames)):
                ...
                if type(row[i]) == FloatType:
                    ...
                    app[key[i]] = row[i].encode('UTF-8','ignore')
                #解析列
                ...
                # if i == 6:
                #     #value = row[i].split('.')[0]
                #     app.append(value.encode('UTF-8','ignore'))
                # else:
                #     app.append(row[i].encode('UTF-8','ignore'))
