...
var obj =  { //定义结构, 2014年9月21日，图片自动回复流水号
      openId:{type:String,required:true,unique:true,index:true}, //应用id
      guid2:{type:Number,required:true,unique:true},  //用户发送图片返回的流水号
...
objSchema.statics.getGuidByOpenid = function (openid, imgurl, cb) { 
      ...
      that.findOne({
            ...
            //如果不存在，先去生成guid
            guidDl.getSpecialGuid('guid2',function(err,guid2){
                  if(err) return cb(err)
                  //然后创建记录
                  that.create({
                        openId:openid,
...
module.exports = mongoose.model('20140921_wxpic', objSchema);